...
| 工具 | 用途 |
|---|---|
| `fishpai_open` | 打开/新建文档并在右侧栏弹出面板。传 `markdown` 新建；传 `path` 打开已有的 `.md`（**不覆盖**已有内容） |
...
| `fishpai_render` | 导出可粘贴/归档用的自包含 HTML 文件（本地图片内嵌 base64） |

标准流程：

1. 用户要写/改一篇公众号文章 → `fishpai_open({ markdown })`（或 `{ path: 'in/x.md' }` 打开现成的）
2. 告诉用户「右侧栏的鱼排面板已打开，你可以直接改字、选中块加批注」，然后**停下等他**
...
- **微信风险主题**：`tech`、`gradient` 用渐变文字（`background-clip: text` + 透明字色），
  微信编辑器可能重写掉渐变导致标题异常；`dark_night` 是深色底，粘进公众号会是一整块深色。
  要发公众号就先提醒用户；需要这些风格时走「导出」拿 HTML。
- **字号**：正文 16px 起步，长文 17px 更好读
- **段落**：段落之间空一行；`breaks` 已开启，单换行即换行
- **图片**：`![alt](相对路径)`；复制到公众号时会自动内嵌 base64，
  但**粘进微信编辑器后仍需在编辑器里重新上传一次**（微信不允许外链图，这是平台限制）
...
- 面板里能看到实时预览、主题/字号/手机宽度切换、批注抽屉、历史回滚
