---
name: fishpai
description: 把 Markdown 排成可直接粘进微信公众号编辑器的内联样式 HTML，并在 DSH 右侧栏的「鱼排」面板里与人协同改稿（13 套主题 + 微信脚注 + 代码块高亮）。当用户要求「公众号排版」「微信排版」「把这篇 Markdown 排好看点」「套个主题」「复制到公众号」「改这篇公众号文章」，或提到鱼排 / FishPai / 墨排 / MoPai / mopai-markdown 时使用；也用于人在鱼排面板里改过稿、加了批注或行内占位之后，需要读懂 diff 再改稿的场合。
---
